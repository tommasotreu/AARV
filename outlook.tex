...
In Figure~\ref{fig:DdDdt} we illustrate this, reproducing Figure 5 from
...
We conclude this section by proposing an ambitious yet in our opinion
feasible roadmap for time delay cosmography in the next decade. This
roadmap aims to achieve $\sim0.5$ precision on time delay distance by
2027, building on the tools and techniques demonstrated in the past 15
years and exploiting the large scale surveys that are currently under
way or planned. It is based on a specific strategy, consisting of
constructing the most precise and accurate models of each lens based
on rich datasets for each system. Specifically, for each system one
needs
...
telescopes and configurations \citep{Gre++13,Col++13,Men++15}.
...
\citep{Jee}.  As discussed in Section XX, this additional 
piece of information could improve the constraints on cosmological
...
year. Completing the second step by 2018 will be challenging and will
require accelerating the modeling effort with respect to previous
work, but it would be desirable to complete this step before the
launch of JWST and (check GAIA?).
...
way based on all the wide field imaging surveys (Agnello, More, etc
etc), and should deliver hundreds of new lensed quasar systems
\citep{O+M10}. Dedicated follow-up, rapid or slow time delays.
